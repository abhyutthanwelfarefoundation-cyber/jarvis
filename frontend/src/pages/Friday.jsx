...
function FridayOrb({ size = 90, pulse = false }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for(let i=0;i<20;i++){const s=(i/20)*Math.PI*2+a,e=s+(Math.PI*2/20)*0.65; ctx.beginPath(); ctx.arc(cx,cy,S*0.42,s,e); ctx.lineWidth=S*0.022; ctx.strokeStyle=C; ctx.globalAlpha=i%4===0?0.15:0.75; ctx.stroke();}
      ctx.globalAlpha=1;
      ...
      ctx.font=`900 ${S*0.13}px Orbitron`; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillStyle='#fff'; ctx.globalAlpha=0.9; ctx.fillText('F',cx,cy); ctx.globalAlpha=1;
...
function Section({emoji,title,content,color,isReading}){
  ...
  return(
    <div style={{marginBottom:10,border:`0.5px solid ${color||C}${isReading?'80':'20'}`,borderRadius:8,overflow:'hidden',boxShadow:isReading?`0 0 10px ${color||C}30`:'none'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 12px',background:`${color||C}${isReading?'18':'08'}`}}>
        <div onClick={()=>setOpen(o=>!o)} style={{flex:1,cursor:'pointer'}}><span style={{fontSize:12,color:color||C,fontFamily:'Orbitron',letterSpacing:1}}>{emoji} {title}</span></div>
      ...
      {open&&<div style={{padding:'10px 12px',background:'rgba(0,5,2,0.8)'}}><p style={{fontSize:13,color:'rgba(180,255,220,0.8)',lineHeight:1.8,whiteSpace:'pre-wrap',userSelect:'text'}}>{content}</p></div>}
    </div>
  );
}
...
export default function Friday({jarvis}){
  ...
  useEffect(()=>{
    ...
    if(!announcedRef.current){
      ...
      setTimeout(()=>{ if(mountedRef.current) speak(lang==='hi'?'FRIDAY online हूँ सर। Business intelligence ready है।':'FRIDAY online sir. Business intelligence ready.'); },600);
    }
    loadHistory();
    return ()=>{ mountedRef.current=false; announcedRef.current=false; window.speechSynthesis?.cancel(); readingRef.current=false; };
  },[]);

  const loadHistory=async()=>{
    try{ const d=await apiCall('/api/friday/history'); if(mountedRef.current) setHistory(Array.isArray(d)?d:[]); }catch{}
  };

  const generateBrief=async()=>{
    setLoading(true); setBrief(null); setError(''); setReadingIndex(-1); setIsReadingAll(false); readingRef.current=false;
  ...
  const speakAndWait=(text)=>new Promise((resolve)=>{
    ...
    utter.lang=lang==='hi'?'hi-IN':'en-IN'; utter.rate=0.95; utter.volume=1;
    const voices=window.speechSynthesis?.getVoices()||[];
    const voice=lang==='hi'?voices.find(v=>v.lang==='hi-IN')||voices.find(v=>v.lang?.startsWith('hi')):voices.find(v=>v.lang==='en-IN')||voices.find(v=>v.lang?.startsWith('en'));
    if(voice) utter.voice=voice;
    utter.onend=()=>setTimeout(resolve,500); utter.onerror=()=>resolve();
    window.speechSynthesis?.speak(utter);
  });

  const readBriefAloud=async(sections)=>{
    if(isReadingAll){ window.speechSynthesis?.cancel(); setIsReadingAll(false); setReadingIndex(-1); readingRef.current=false; return; }
    setIsReadingAll(true); readingRef.current=true;
    await speakAndWait(lang==='hi'?'Sir, aapki daily intelligence brief present kar rahi hoon.':'Sir, presenting your daily intelligence brief.');
    for(let i=0;i<sections.length;i++){
      if(!readingRef.current) break;
      setReadingIndex(i);
      const s=sections[i];
      const fullText=(lang==='hi'?`Section ${i+1}. ${s.title}. `:`${s.title}. `)+s.content.replace(/\*\*/g,'').replace(/\*/g,'').replace(/#{1,3}/g,'').replace(/₹/g,' rupees ').replace(/\$/g,' dollars ').replace(/\n- /g,'. ').replace(/- /g,'').replace(/\n+/g,'. ').trim();
      await speakAndWait(fullText);
      if(!readingRef.current) break;
    }
    if(readingRef.current) await speakAndWait(lang==='hi'?'Brief complete ho gayi sir. Aaj ka din productive ho.':'Brief complete sir. Have a productive day.');
  ...
  const doSearch=async()=>{
    ...
    try{
      const d=await apiCall('/api/friday/search',{method:'POST',body:JSON.stringify({query:searchQ,lang})});
          ...
          <div style={{padding:'14px'}}>
            {!brief&&!loading&&(
              <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:12,padding:'20px 0'}}>
                ...
                {error&&<div style={{background:'rgba(255,51,85,0.08)',border:'0.5px solid #ff335540',borderRadius:6,padding:'10px 14px',fontFamily:'Share Tech Mono',fontSize:10,color:'#ff3355',textAlign:'center',maxWidth:280}}>{error}</div>}
              ...
              <div>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
                  ...
                  <div style={{display:'flex',gap:6}}>
                    <button onClick={()=>readBriefAloud(parseBrief(brief.brief))} style={{padding:'6px 14px',background:isReadingAll?`${C}25`:`${C}10`,border:`1px solid ${C}`,borderRadius:5,color:C,fontSize:9,fontFamily:'Orbitron',cursor:'pointer',letterSpacing:1,animation:isReadingAll?'blink 1.5s infinite':'none'}}>
                ...
                {parseBrief(brief.brief).map((s,i)=>(<Section key={i} emoji={s.emoji} title={s.title} content={s.content} color={s.color} isReading={readingIndex===i}/>))}
                {isReadingAll&&<div style={{textAlign:'center',padding:'10px',fontFamily:'Share Tech Mono',fontSize:9,color:`${C}50`,letterSpacing:1}}>READING SECTION {readingIndex+1} OF {parseBrief(brief.brief).length}...</div>}
              </div>
            )}
          </div>
        )}
          ...
          <div style={{padding:'14px'}}>
            ...
            {selected?(
              <div>
                ...
                {parseBrief(selected.brief).map((s,i)=>(<Section key={i} emoji={s.emoji} title={s.title} content={s.content} color={s.color} isReading={false}/>))}
              </div>
            ):history.length===0?(
